	...
	<?php if ( $quiz ) : ?>
		...
		<form method="POST" action="" name="llms_start_quiz" enctype="multipart/form-data">
			...
			<?php if ( $quiz->is_open() ) : ?>
				...
				<button class="btn" id="llms_start_quiz" name="llms_start_quiz" type="submit">
			...
			<?php if ( $lesson->get_next_lesson() && llms_is_complete( get_current_user_id(), $lesson->get( 'id' ), 'lesson' ) ) : ?>
				<a href="<?php echo get_permalink( $lesson->get_next_lesson() );?>" class="btn"><?php _e( 'Next Lesson','lifterlms' ); ?></a>
			<?php endif; ?>
		</form>
